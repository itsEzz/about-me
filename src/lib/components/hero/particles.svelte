<script lang="ts">
	import type { Circle } from '$lib/types/particles';
	import { mode } from 'mode-watcher';
	import { onMount } from 'svelte';

	// Config variables
	const quantity: number = 200;
	const staticity: number = 80;
	const ease: number = 50;
	const size: number = 0.4;
	const vx: number = 0;
	const vy: number = 0;

	// States & Variables
	let color = $derived<string>(mode.current === 'dark' ? '#ffffff' : '#000000');
	...
	function drawCircle(circle: Circle, update: boolean = false) {
		if (context) {
...
<div class="absolute inset-0 -z-10" bind:this={canvasContainerRef} aria-hidden="true">
